...
export default function NappinAiDataScreen({ navigation }) {
  const { getAdvancedSleepData, updateAdvancedSleepData } = usePersonalization();
  
  // Load saved data
  const savedData = getAdvancedSleepData();
  
  const [sleepTimeLow, setSleepTimeLow] = useState(savedData.sleepLatencyLow || '5');
  const [sleepTimeHigh, setSleepTimeHigh] = useState(savedData.sleepLatencyHigh || '10');
  const [heartRate, setHeartRate] = useState(savedData.restingHeartRate ? savedData.restingHeartRate.toString() : '70');
  const [bedtime, setBedtime] = useState(savedData.bedtime || '10:30 PM');
  const [wakeTime, setWakeTime] = useState(savedData.wakeTime || '6:30 AM');
  const [sleeperType, setSleeperType] = useState(savedData.sleeperType || 'Average');
  const [difficulty, setDifficulty] = useState(savedData.difficultyFallingAsleep || 0.5); // 0-1 range for slider
  ...
  const handleSave = async () => {
    try {
      ...
      // Save sleep data using PersonalizationProvider
      await updateAdvancedSleepData({
        sleepTimeLow,
        sleepTimeHigh, 
        ...
        sleeperType,
        difficulty,
        sleepDuration: actualSleepDuration.toString()
      });
      
      console.log('Advanced sleep data updated:', {
      ...
      Alert.alert(
        'Data Saved',
        'Your advanced sleep data has been updated successfully.',
        [
          {
            text: 'OK',
      ...
      console.error('Error saving advanced sleep data:', error);
      Alert.alert(
        'Error',
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Nappin AI Data</Text>
        </View>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.titleContainer}>
            <Text style={styles.mainTitle}>Edit your sleep data.</Text>
          </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>Changes are saved immediately and applied to future naps.</Text>
        </View>
      </ScrollView>
