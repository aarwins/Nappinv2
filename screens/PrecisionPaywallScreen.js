...
export default function PrecisionPaywallScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        setLoading(true);
        // Fetch products for apple_watch device type
        const fetchedProducts = await fetchProductsForDevice('apple_watch');
        setProducts(fetchedProducts);
      } catch (error) {
        console.error('[PrecisionPaywallScreen] Error loading products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const yearlyProduct = products.find((p) => p.sku === 'precision_yearly');
  const monthlyProduct = products.find((p) => p.sku === 'precision_monthly');
